...
def boundary_condition_origin(model,x, t):
    ...
    return bc_loss_origin

# ---- Training Loop ----
def train(model, epochs=40000, n_collocation=200, n_boundary=500, n_initial=500, lr=1e-3, print_every=2000):
    ...
    for epoch in range(1, epochs+1):
        ...
        loss_bc0 = boundary_condition_origin(model, x_b0, t_b)
        u_b1 = model(x_b1, t_b)
        loss_b = loss_bc0 + (u_b1**2).mean()

        # Initial condition loss
        x_i, t_i = sample_initial(n_initial)
        u_i = model(x_i, t_i)
        u_exact_i = torch.exp(-((x_i - 0.3)**2) / (2 * 0.05**2))  # Example initial condition
        loss_i = torch.mean((u_i - u_exact_i)**2)

        # Total loss
        loss = loss_phys + loss_b + loss_i

        loss.backward()
        optimizer.step()

        if epoch % print_every == 0 or epoch == 1:
            print(f"Epoch {epoch:5d} | Loss: {loss.item():.3e} | Loss_phys: {loss_phys.item():.3e} | Loss_b: {loss_b.item():.3e} | Loss_i: {loss_i.item():.3e}")
...
if __name__ == "__main__":
    ...
    if train_flag:
        ...
        model = train(model, epochs=4000, n_collocation=500, n_boundary=100, n_initial=100, lr=1e-3, print_every=500)
        ...
        for t in t_values:
            t_plot = torch.full_like(x_plot, t).to(device)  # Create a constant t for all x
            with torch.no_grad():
